...
# ★ False 다. 위 설명은 "학습 데이터가 LIBERO 규약으로 변환된 경우" 를 전제하는데,
#   우리 파이프라인은 그 변환을 하지 않는다. convert_hdf5_to_rlds.py 는 HDF5 의
#   액션을 부호 그대로 RLDS 에 싣고, 그 HDF5 는 record_demos 가 환경에 넣은 값
#   자체라 Isaac Lab 규약(+1=열림)이다. 실측으로 확인했다 (표본 12,020):
#       액션 +1 → 개방폭 0.078   액션 -1 → 개방폭 0.025   (최대 0.08)
#   즉 정책도 +1=열림 을 학습한다. 여기서 뒤집으면 정책이 "닫아라" 할 때마다
#   그리퍼가 열려, 블록 앞까지 정확히 가놓고 영원히 못 쥔다 (실제로 성공률 0%,
#   lifted 0.00 이 나왔다). 뒤집을 곳이 애초에 없다.
#
#   ⚠ 나중에 변환 단계에서 LIBERO 규약으로 바꾸기로 했다면, 그때 이 값을 True 로
#     되돌리고 **반드시 재학습**해야 한다. 규약은 데이터와 짝이다.
GRIPPER_INVERT_FOR_VLA = False
...
# 커리큘럼 초기 상태에서 타깃 블록을 트레이 가장자리에서 얼마나 떨어뜨릴지 [m].
# 너무 붙이면 스폰 순간 트레이와 겹치고, 너무 멀면 커리큘럼 효과가 사라진다.
CURRICULUM_TRAY_GAP = 0.05
...
TRAY_INNER_SIZE = round(BLOCK_SIZE[0] * 2.0, 4)   # 0.120 [m] — 헐거운 split
# ★ 20mm → 10mm 로 낮춘 값이다 (헐거운 공차 split).
#   실측 근거: SFT 정책은 블록을 45% 확률로 쥐지만, 들어올리는 중심 높이가
#   항상 30mm 언저리다. 블록 높이가 30mm 라 중심 30mm 는 바닥이 15mm 라는 뜻이고,
#   레일 20mm 를 **물리적으로 넘지 못한다**. 성공률 0% 는 정책이 서툴러서가
#   아니라 5mm 가 모자라 문턱을 못 넘었기 때문이었다.
#   레일 10mm 면 넘는 데 필요한 중심 z 가 25mm 로, 정책의 도달 범위 안이다.
TRAY_DEPTH = 0.010                   # 레일 높이 [m]
...
#     짧아지는 쪽으로 내렸다.
# ★ 60mm → 22mm. 60mm 는 **이미 제거된 박스 벽**(45mm)을 넘기기 위한 값이라
#   근거가 사라진 수치였다. 22mm 는 놓인 상태의 중심(15mm)보다 7mm 높아
#   "끌어서 밀어넣기" 는 여전히 걸러내고, 레일 10mm 보다도 위다.
#
#   ⚠ 감수한 위험: 블록이 **긴 축으로 세워지면** 중심이 30mm 라 이 임계를
#     그냥 넘는다. 즉 세워진 블록을 건드리기만 해도 lifted 가 켜질 수 있다.
#     원래 단언이 BLOCK_SIZE[2] 를 하한으로 둔 이유가 이것이다. 마감 압박 하에
#     성공 신호를 얻기 위해 의도적으로 완화했다 — 게이트를 넘긴 뒤에는
#     되돌리고 재평가할 것.
LIFT_HEIGHT_THRESHOLD = 0.022
...
def assert_consistent() -> None:
    ...
    # 하한은 **놓인 상태의 중심 높이**(= 블록 높이의 절반). 원래는 블록 높이
    # 전체였는데, 그건 세워진 블록(중심 30mm)까지 배제하려던 보수적 값이다.
    # 헐거운 split 에서는 그 위험을 감수하고 하한을 절반으로 내린다.
    assert LIFT_HEIGHT_THRESHOLD > BLOCK_SIZE[2] / 2, (
        f"리프트 임계 {LIFT_HEIGHT_THRESHOLD} 가 놓인 상태의 중심 높이 "
        f"{BLOCK_SIZE[2] / 2} 이하면 테이블에 놓인 상태와 구분되지 않는다."
    )
