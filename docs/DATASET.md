...
## ~~미해결 1: 궤적 길이~~ → 8Hz 다운샘플로 해소 (2026-08-13)
...
**택한 것: 3번의 변형 — 재수집 대신 시간축 다운샘플.**

우리 제어 주기 24Hz 는 VLA 학습 데이터로 높은 편이다 (OXE 는 대체로 3~15Hz,
LIBERO 는 20Hz). 스텝당 평균 이동이 2mm 남짓이라 인접 프레임이 거의 같다 —
정보량 대비 프레임이 과했다. `scripts/downsample_demos.py` 로 3배 솎아 8Hz 로
낮췄다.

| | 24Hz | 8Hz |
|---|---|---|
| 궤적 길이 | 598스텝 (예산 300 초과) | **202스텝** (1500개 전부 예산 안) |
| 총 프레임 | 897,712 | 298,776 |
| SFT 에폭 | 0.36 | **1.07** |
| RLDS 크기 | 5.91 GB | **1.97 GB** |
| 청크 8 커버 | 0.33초 | **1.0초** (OXE/LIBERO 범위) |

★ 액션은 델타 포즈라 **k번째만 남기면 안 된다** — 구간 델타를 합산해야 한다.
  검증: 누적 이동이 오차 4e-6 으로 보존되고, RLDS 의 action q99 가 24Hz 대비
  2.98~3.00배로 나왔다 (3스텝을 합쳤으니 3배가 정답).

★ 그리퍼는 이진이라 합산 대신 구간 마지막 값을 쓴다. 표본 300 에피소드 측정:

| | 값 |
|---|---|
| 전환 보존율 | **100.0%** (원본 평균 2.74회 → 동일) |
| 한 창에 2회 이상 전환된 창 | **0 / 59,336** |
| 닫힘 시간 비율 | 60.17% → 59.93% (편차 0.24%p) |
| 전환 시점 | 최대 2스텝(83ms) 앞당겨짐, 평균 약 1스텝 |

  전환이 하나도 사라지지 않았다 — 물리 그리퍼가 125ms 안에 두 번 상태를 바꿀
  일이 없기 때문이다. 파지 성공률이 유독 낮으면 "구간 첫 값" 규칙으로 바꿔 볼
  수 있다 (코드 한 줄, 듀티 편차는 0.228%p 로 거의 같다).

⚠ **다운샘플 결과물은 재생용이 아니다.** 합산된 델타를 한 스텝에 적용하면 IK
  추종 한계를 넘고, `states`/`initial_state` 도 넣지 않았다. replay·annotate 에는
  원본 `generated.hdf5` 를 쓸 것.

⚠ **환경도 8Hz 로 맞춰야 한다.** `VLA_DECIMATION=15` (1/120 × 15 = 0.125초).
  안 주면 8Hz 용 델타가 24Hz 에 적용돼 3배 빠르게 움직인다 — 평가·RFT 가 통째로
