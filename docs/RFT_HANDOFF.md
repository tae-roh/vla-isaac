...
**`eval_every` 는 구현돼 있지 않다.** `GRPOConfig` 에 필드는 있지만 학습 루프
어디에서도 쓰이지 않는다 (`save_every` 만 쓰인다). 설정에 값을 적어도 학습 중
평가가 돌지 않으므로, `save_every` 로 저장된 체크포인트를 `eval_rollout.py` 로
따로 재야 커브가 나온다.

**YAML 오타가 조용히 무시된다.** `load_config` 는 모르는 키를 `extra` 에 담고
경고하지 않는다. 키 이름을 틀리면 기본값으로 돌아가는데 알 방법이 없다.
(현재 두 설정 파일의 키는 전부 인식됨을 확인했다.)

### 보상 임계가 왜 1e-6 인가

`RewardManager.compute` 는 `func() * weight * dt` 를 돌려준다
(IsaacLab/.../reward_manager.py:150). 가중치 있는 항이 `success` 하나뿐이고
`success_bonus` 는 0.0/1.0 이므로, 성공 시 env 가 주는 값은

    1.0 × 1.0 × step_dt = 0.125   (8Hz. 24Hz 였다면 0.042)

다. **0.5 임계로는 성공이 영원히 안 잡히는데 에러도 안 난다.** 부호만 보는
`> 1e-6` 이라 decimation 을 바꿔도 안전하다. 절대값으로 되돌리지 말 것.
