...
## 7. 현재 성능 (SFT, 40 에피소드, 헐거운 split)

```
파지         15%      (엄격한 판정 기준 적용 후)
리프트       15%
트레이 진입  2.5%     (원래 분포) / 50% (커리큘럼)
성공         0%       ← 단 reward>0.5 버그로 애초에 검출 불가였다
```

⚠ **성공률 0% 는 재측정이 필요하다.** 위 표의 모든 성공률은
`success_latch |= reward > 0.5` 버그가 살아 있던 상태에서 잰 값이다. 병합으로
`> 1e-6` 이 들어왔으므로, **RFT 착수 전에 40 에피소드를 다시 재고 진짜 기준선을
확보할 것.** 게이트(30%)를 이미 넘고 있을 가능성도 배제할 수 없다.
